...
const uint8_t SYS_INT_SEND_REQUEST_REJECTED = 0x80;
const uint8_t SYS_INT_SEND_START 			= 0x40;
const uint8_t SYS_INT_SEND_END 				= 0x20;
const uint8_t SYS_INT_POST_ACCESS			= 0x10;
const uint8_t SYS_INT_RECEIVED_DATA			= 0x08;
const uint8_t SYS_INT_RECEIVED_INFORMATION	= 0x04;
const uint8_t SYS_INT_ABNORMAL_EXIT			= 0x02;
const uint8_t SYS_INT_PHYSICAL_LAYER		= 0x01;
...
static volatile int rxDataType;
static volatile int rxSyncClass;
static volatile bool rxCRCStatus;
static volatile int rpi;
...
inline static void HRC6000SysPostAccessInt()
{
	...
	*/
	SEGGER_RTT_printf(0, "%d SYS_INT_POST_ACCESS 0x10\n",PITCounter);
	// InterLateEntry interrupt
...
inline static void HRC6000SysReceivedInformationInt()
{
	...
	*/
	...
	SEGGER_RTT_printf(0, "\tcc:%d\tDataType:%d\tSyncClass:%d\tTC:%d\n",rxColorCode,rxDataType,rxSyncClass,(tmp_val_0x52 & 0x04>>2));
	...
	switch(slot_state)
	{
		case DMR_STATE_IDLE:
			...
			if (rxColorCode == trxGetDMRColourCode())
			{
...
//				SEGGER_RTT_printf(0, ">>> Audio frame %02x\r\n",rxdt & 0x07);
				...
				store_qsodata();
				if (slot_state == DMR_STATE_RX_1)
				{
					SEGGER_RTT_printf(0,"%d\tAudio\t%d\n",PITCounter,slot_state);
	...
	write_SPI_page_reg_byte_SPI0(0x04, 0x83, tmp_val_0x82);  //Clear all interrupt Flags that caused this interrupt
...
static inline void HRC6000TimeslotInterruptHandler()
{
	...
	SEGGER_RTT_printf(0, "%d\tTS_ISR\ts:%d\tr0x52:0x%02x\tr0x42:0x%02x\n",PITCounter,slot_state,((tmp_val_0x52 & 0x04) >> 2),tmp_val_0x42);
	// RX/TX state machine
	switch (slot_state)
	{
